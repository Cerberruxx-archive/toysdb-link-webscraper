// Imports cheerio library and 'createWriteStream' function from node

import * as cheerio from 'cheerio';
import {createWriteStream} from 'node:fs';

// Declare url array to iterate through with loop
const urls = [
...
async function scrape(urlList, fname) {
	// Create the stream (opens the file once)
	/*
	'createWriteStream' creates a pointer to the file, opening a small route to the hard drive instead of loading into the memory.
	The flag tells node 'if file exist, start writing at the very bottom and keep what's already there.'
	*/
	...
	for (const url of urlList) {
		try {
			...
			/*
			'fetch(url) sends a request to the site, and 'await' tells the script to wait until the website responds before moving to the next line.
			'.text()' converts the raw data from the site into a long string of HTML code (how?)
			'cheerio.load(html)' turns the messy string into a searchable object (stored as $). It allows you to use CSS selectors to search elements from the domain
			...
			*/
			...
			/*
			$('title').text() find the <title> tag and grabs the text inside; second part is regex that removes the newline characters inside the title (without this, a single title could break into two lines and ruin the formatting)
			|| 'No Title' is a fallback, providing a default string if the script doesn't fetch the tag correctly.
